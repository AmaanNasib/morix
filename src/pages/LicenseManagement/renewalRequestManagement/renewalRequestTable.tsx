...
import { Eye, Monitor, Send, X } from "lucide-react";
import { useCallback, useEffect, useRef, useState } from "react";

import { renewalRequestColumns } from "./renewalRequestColumnConfig";
...
import { renewalRequestData } from "@/data/jsonData";
// import { renewalRequestColumns } from "@/data/tableData";
import useTableControls from "@/hooks/useTableControls";


interface RenewalRequestTableProps { }


export const RenewalRequestTable = ({ }: RenewalRequestTableProps) => {
    ...
    const {
        ...
        filterValue,
    } = useTableControls(renewalRequestData, renewalRequestData.length);
    ...
    const fetchData = useCallback(
        async (searchQuery: string = '', searchPage: number) => {
            try {
                ...
                setLicenseData(renewalRequestData);
                if (search) {
                    const filteredData = renewalRequestData.filter((item: any) =>
    ...
    const fetchDropdownData = async (
        ...
        try {
            ...
            if (result && result.data && Array.isArray(result.data)) {
                ...
                const dropdownValuesCustom = result.data.map((item: { id: any; name: any; lastName: any; email: any; level: any; label: any; value: any; }) => ({
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 rounded-[16px] p-4 border border-gray-200 bg-gray-100">
                ...
                <div className="w-full">
                    ...
                    <ReactSelectDropdown
                        ...
                        onMenuOpen={() => fetchDropdownData("status", "null")}
                    />
                </div>
